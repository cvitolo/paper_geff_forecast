...
```{r prepFigure3, echo = FALSE, message = FALSE, results = 'hide', cache = TRUE}
...
forecasts_folder <- "/hugetmp/FireEventsDatabase/forecasts/fwi/"

df_used <- data.frame(matrix(NA, nrow = 0, ncol = 13))
names(df_used) <- c("id", "lat", "long", "region", "date", "OBS", "ERA5",
                    "HRES_d2", "HRES_d6", "HRES_d10", "CRPS_d2", "CRPS_d6", "CRPS_d10")

for (i in seq_along(dates2017)[216:365]){
  ...
  if (day10 %in% dates2017){
    
    # Day2
    HRES2 <- raster::brick(file.path(forecasts_folder,
                                     paste0("ECMWF_FWI_",
                                            gsub("-", "",
                                                 as.character(day2)),
                                            "_1200_hr_fwi.nc")))[[2]]
    dfx$HRES_d2 <- raster::extract(x = HRES2, y = spdf)
    # Day6
    HRES6 <- raster::brick(file.path(forecasts_folder,
                                     paste0("ECMWF_FWI_",
                                            gsub("-", "",
                                                 as.character(day6)),
                                            "_1200_hr_fwi.nc")))[[6]]
    dfx$HRES_d6 <- raster::extract(x = HRES6, y = spdf)
    # Day10
    HRES10 <- raster::brick(file.path(forecasts_folder,
                                      paste0("ECMWF_FWI_",
                                             gsub("-", "",
                                                  as.character(day10)),
                                             "_1200_hr_fwi.nc")))[[10]]
    dfx$HRES_d10 <- raster::extract(x = HRES10, y = spdf)
    rm(HRES2, HRES6, HRES10)
    
    # Remove NAs
    dfx <- dfx %>% filter(complete.cases(.))
    ...
    mat2 <- matrix(NA, nrow = nstations, ncol = 51)
    mat6 <- matrix(NA, nrow = nstations, ncol = 51)
    mat10 <- matrix(NA, nrow = nstations, ncol = 51)
    for (j in 1:51){
    
      # Extract the modelled FWI from ENS
      ens_member <- sprintf("%02d", j - 1)
      # Day2
      ENS2 <- raster::brick(file.path(forecasts_folder,
                                       paste0("ECMWF_FWI_",
                                              gsub("-", "",
                                                   as.character(day2)),
                                              "_1200_", ens_member,
                                              "_fwi.nc")))[[2]]
      mat2[, j] <- raster::extract(x = ENS2, y = spdf)
      # Day6
      ENS6 <- raster::brick(file.path(forecasts_folder,
                                       paste0("ECMWF_FWI_",
                                              gsub("-", "",
                                                   as.character(day6)),
                                              "_1200_", ens_member,
                                              "_fwi.nc")))[[6]]
      mat6[, j] <- raster::extract(x = ENS6, y = spdf)
      # Day10
      ENS10 <- raster::brick(file.path(forecasts_folder,
                                       paste0("ECMWF_FWI_",
                                              gsub("-", "",
                                                   as.character(day10)),
                                              "_1200_", ens_member,
                                              "_fwi.nc")))[[10]]
      mat10[, j] <- raster::extract(x = ENS10, y = spdf)
    }
    # Populate CRPS for Day 2
    dfx$CRPS_d2 <- veriApply(verifun = "EnsCrps", fcst = mat2, obs = dfx$OBS)
    # Populate CRPS for Day 6
    dfx$CRPS_d6 <- veriApply(verifun = "EnsCrps", fcst = mat6, obs = dfx$OBS)
    # Populate CRPS for Day 10
    dfx$CRPS_d10 <- veriApply(verifun = "EnsCrps", fcst = mat10, obs = dfx$OBS)
...
d <- df_used %>%
  ...
  reshape2::melt(id.vars = c("fwiobs", "modelled_type", "modelled_value"))
