...
forecasts_folder <- "/hugetmp/forecasts/fwi"
...
```{r prep, echo = FALSE, message = FALSE, results = 'hide', cache = TRUE}
...
# 
# df_used <- data.frame(matrix(NA, nrow = 0, ncol = 37))
# names(df_used) <- c("id", "lat", "long", "region", "date", "OBS", "ERA5",
#                     paste0(c("CLIM_d", "HRES_d", "CRPS_d"), rep(1:10, each =3)))
# 
...
# 
#     # Get ERA5 data
#     message(paste("Handling ERA5 for", lead_day))
#     dayx <- which(format(datesERA5, "%m-%d") %in% format(lead_day, "%m-%d")
#                   & lubridate::year(datesERA5) != "2017")
#     system(paste0("cdo seltimestep,", paste0(dayx, collapse = ","),
#                   " /scratch/rd/nen/perClaudia/era5/fwi_1980_2019.nc ",
#                   "/scratch/rd/nen/perClaudia/era5/fwi_1980_2019_dayx.nc"),
#            ignore.stderr = TRUE)
#     ERA5 <- brick("/scratch/rd/nen/perClaudia/era5/fwi_1980_2019_dayx.nc")
...
# 
#     # Get ENS
#     df_ens <- matrix(NA, nrow = nstations, ncol = 51)
#     message(paste("Handling ENS for", dates2017[i], "day", leadtime))
#     for (j in 1:51){
...
#                                             "_1200_", ens_member,
#                                             "_fwi.nc")))[[leadtime]]
#       df_ens[, j] <- raster::extract(x = rotate(ENS), y = spdf)
#     }
# 
#     # Remove rows with NAs and update spatial points
#     if (leadtime == 1){
#       rows2remove <- which(rowSums(is.na(df_era5)) > 0)
#       if (length(rows2remove) > 0){
#         df_ens <- df_ens[-rows2remove, ]
#         df_hres <- df_hres[-rows2remove]
#         df_era5 <- df_era5[-rows2remove, ]
#         # Resample ERA5 to get 39 random ensemble members
#         df_era5 <- df_era5[, sample(x = 1:dim(df_era5)[2], size = 39,
#                                     replace = TRUE)]
# 
#         dfx <- dfx[-rows2remove, ]
#         # Re-define spatial point
#         spdf <- dfx[, c("id", "lat", "long")]
#         # Promote data.frame to spatial
#         coordinates(spdf) = ~long + lat
#         crs(spdf) <- new("CRS",
...
# 
#     # Populate CRPS
#     df_crps <- veriApply(verifun = "EnsCrps", fcst = df_ens, obs = df_era5)
#     df_crpss <- apply(X = df_crps, MARGIN = 1, FUN = mean)
#     df_era5_mean <- apply(X = df_era5, MARGIN = 1, FUN = mean)
# 
#     dfx <- cbind(dfx, df_era5_mean, df_hres, df_crpss)
#   }
# 
#   names(dfx) <- names(df_used)
# 
#   # Append to table
...
df_used <- readRDS("../data/df_geff_era5_hres_crps.rds")

# Promote data.frame to spatial
coordinates(df_used) = ~long + lat
...
# Synop stations of interest
crs(df_used) <- crs(BasisRegions)
synops <- spatialEco::point.in.poly(df_used, BasisRegions)
# saveRDS(synops, "../data/synops.rds")
```
...
f3 <- data.frame(synops) %>%
  select(Region, CRPS_d1, CRPS_d2, CRPS_d3, CRPS_d4, CRPS_d5, CRPS_d6,
         CRPS_d7, CRPS_d8, CRPS_d9, CRPS_d10) %>%
...
```{r prepFig4, echo = FALSE, message = FALSE, results = 'hide', cache = TRUE}
# Get ERA5
ERA5 <- brick("/scratch/rd/nen/perClaudia/era5/fwi_1980_2019.nc")
# Get FRP
                  ...
                  .options.snow = opts) %dopar%
{
  ...
  hres <- raster::rotate(raster::brick(myfilelist[[i]]))
  x <- raster::extract(x = hres, y = spdf)
  x <- data.frame(x)
  names(x) <- paste0("Day", 1:10)
  
  # Get ERA5 to calculate the very high warning levels
  very_high <- caliver::daily_clima(r = ERA5, dates = dates2017[i], probs = 0.95)
  thr95 <- raster::extract(x = very_high, y = spdf)

  # Append results to table
  df <- cbind(spdf@coords, spdf@data, x, thr95)
